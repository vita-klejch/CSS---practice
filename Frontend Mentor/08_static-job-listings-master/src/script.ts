...
function displayCards(cardsData: cardType[], cssPrefix: string) {
  cardsData.forEach((card) => {
    const tagsArray: string[] = [card.role, card.level, ...card.languages];
    ...
    const cardHeader = getStyledElement(
      "div",
      cssPrefix + "__header",
      card.company
    );
    ...
    if (card.new) {
      const cardNEWinfo = getStyledElement(
        "span",
        cssPrefix + "__header--bubble",
        "New!"
      );
    ...
    if (card.featured) {
      ...
      const cardNEWinfo = getStyledElement(
        "span",
        cssPrefix + "__header--bubble-black",
        "Featured"
      );
      cardHeader.appendChild(cardNEWinfo);
    }
    const cardHeadline = getStyledElement(
      "div",
      cssPrefix + "__headline",
      card.position
    );
    cardMainInfo.appendChild(cardHeadline);
    const cardJobComments = getStyledElement(
      "div",
      cssPrefix + "__job-comments"
    );
    const cardComment1 = getStyledElement(
      "div",
      cssPrefix + "__single-comment",
      card.postedAt
    );
    const cardComment2 = getStyledElement(
      "div",
      cssPrefix + "__single-comment",
      card.contract
    );
    const cardComment3 = getStyledElement(
      "div",
      cssPrefix + "__single-comment",
      card.location
    );
...
function addToActiveTags(tag: string) {
  if (!activeTags.has(tag)) {
    ...
    infoBox?.appendChild(boxItem);
    activeTags.size == 1 &&
      infoBox?.parentElement?.classList.remove("info-box--hidden");
    applyFilter();
  }
}

// removes ONE filter from sected filters/tags
function removeFromActiveTag(element: HTMLElement) {
  ...
  activeTags.size == 0 &&
    infoBox?.parentElement?.classList.add("info-box--hidden");
  applyFilter();
}
